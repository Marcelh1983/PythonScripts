...
def get_unique_file(filename: str, seq=0):
    ...
    if (os.path.isfile(dst)):
        ...
        return get_unique_file(filename, seq + 1)
    else:
        return dst
...
def handle_image(filename: str, images: list):
    dst = get_unique_file(os.path.basename(filename))
...
def start():    
    ...
    output_folder = 'c://tmp//diggel_shutterstock'
    ...
    for filename in glob.iglob(input_folder + '/**/*.png', recursive=True):
        file = handle_image(filename, images)
        images.append(file)
